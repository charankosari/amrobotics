...
const AdminProducts = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const response = await axios.get(
        "http://localhost:5080/api/v1/productsHome",
        {
          headers: {
  ...
  const handleDeleteProduct = async (productId) => {
    try {
      ...
      const response = await axios.delete(
        `http://localhost:5080/api/v1/product/${productId}`,
        {
          headers: {
            Authorization: `Bearer ${authToken}`,
          },
  ...
  return (
    <div
      className="container mx-auto p-4 sm:w-[80%] w-[70%] h-[100vh] overflow-y-auto block"
      id="scroll"
    >
      ...
      <div className="flex sm:flex-row flex-col sm:gap-12 gap-1 ml-12 sm:ml-0 items-center ">
        ...
        <div className="flex flex-row gap-1 justify-center items-center">
          <input
            type="text"
            className="w-[200px] h-[34px] p-4 bg-white text-black rounded-md border "
            onChange={(e) => {
          ...
          <button
            className="bg-yellow-500 p-[5px] rounded-md text-white"
            onClick={clearSearch}
          >
            clear
          </button>
      ...
      <div>
        <button
          className="btn bg-green-600 text-white mb-2"
